...
def main(transport='streamable-http') -> int:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"  DB_PASSWORD: {'***set***' if db_config['db_password'] else 'NOT SET'}")
        ...
        logger.info(f"  ODOO_PASSWORD: {'***set***' if os.getenv('ODOO_PASSWORD') else 'NOT SET'}")
        ...
        async def asetup_checkpointer():
            ...
            try:
                ...
                masked_uri = db_config['pg_uri'].replace(db_config['db_password'], '***')
                logger.info(f"Using PostgreSQL URI: {masked_uri}")
                
                async with AsyncPostgresSaver.from_conn_string(db_config['pg_uri']) as saver:
                    await saver.setup()
